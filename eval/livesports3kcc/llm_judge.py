...
from baselines.livecc.utils.multiprocessor import local_mt
from openai import OpenAI
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('--baseline_id', type=str, required=True, help='Baseline model id')
parser.add_argument('--baseline_jsonl', type=str, required=True, help='Baseline model predictions in JSONL format')
...
gpt = OpenAI(api_key=os.environ.get('OPENAI_KEY'))
    ...
    while True:
        try:
            ab_resp = gpt.chat.completions.create(
                model='gpt-4o-mini',          # Public model name
...
if __name__ == '__main__':
    ...
    for video_event_id in video_event_id_to_baseline_pred:
        if video_event_id  not in video_event_id_to_model_pred or video_event_id_to_model_pred[video_event_id].strip() == '':
            video_event_id_to_baseline_pred[video_event_id] = ""
            video_event_id_to_model_pred[video_event_id] = ""
            print(f'Missing prediction for {video_event_id}')
